...
function logger(func, logFunc) {
  return (...args) => {
    const template = `${func.toString().slice(func.toString().indexOf(' ') + 1, func.toString().indexOf('('))}(${args.map((arg) => JSON.stringify(arg)).join(',')})`;
...
function partialUsingArguments(fn, ...args1) {
  ...
  function checkArgs(...args) {
    ...
    return (...args2) => checkArgs(...args, ...args2);
  }
  return checkArgs(...args1);
}
